...
export const titleParser = async (driver: WebDriver) => {
  let title = await driver.findElement(By.className("Fc1rA")).getText();
  return title;
};

// 가게 업종 가져오기
export const categoryParser = async (driver: WebDriver) => {
  let category = await driver.findElement(By.className("DJJvD")).getText();
  return category;
};

// 가게 주소 가져오기
export const addressParser = async (driver: WebDriver) => {
  let address = await driver.findElement(By.className("LDgIH")).getText();
  return address;
};

// 네이버 정식 지도 url 가져오기
export const shareMapUrlParser = async (driver: WebDriver) => {
  let shareButtons = await driver.findElement(
    By.css(".D_Xqt.naver-splugin.spi_sns_share")
  );
  if (shareButtons) {
...
export const visitorReviewCountParser = async (driver: WebDriver) => {
  try {
    const review_section = await driver.findElements(
      By.css(".PXMot a.place_bluelink em")
    );
...
export const menuTextParser = async (
  driver: WebDriver,
  index: number,
  menusArr: any[]
) => {
...
export const companyProvidedPicturesParser = async (driver: WebDriver) => {
  ...
  if (pictureMenus === "업체사진") {
    ...
    const picture = await driver.findElements(
      By.css(".wzrbN .place_thumb > img")
    );
