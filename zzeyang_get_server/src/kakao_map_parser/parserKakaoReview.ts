...
export const parserKakaoReview = async (
  driver: WebDriver,
  data: any,
  type: string
) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const addressKeyword =
        addressSplit[0] + " " + addressSplit[1] + " " + addressSplit[2];
      const titleKeyword = getTitleKeyword(data.title);

      const searchKeyword = `${addressKeyword} ${titleKeyword}`;
      await driver.get(
        KAKAO_MAP_URL + `?q=${encodeURIComponent(searchKeyword)}`
      );
      await driver.sleep(4000);

      // 가게 평점 가져오기
      try {
        const score = await driver
          .findElement(By.css(".score em.num"))
          ?.getText();
      ...
      //가게 리뷰 갯수 가져오기
      try {
        const reviewCount = await driver
          .findElement(By.css("a.review em"))
          ?.getText();
      ...
      //가게 공유 링크 가져오기
      try {
        const kakao_detail_url_el = await driver.findElement(
          By.css(".contact.clickArea a")
        );
